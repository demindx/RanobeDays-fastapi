...
<template>
	<AppPanel as="section" class="rounded-2xl space-y-3">
		...
		<AppEmptyState v-if="!props.chapters.length" compact>
			Глав пока нет.
		</AppEmptyState>
		...
		<div v-else class="space-y-2">
			<button
				...
				type="button"
				:class="[
					'flex w-full cursor-pointer items-center justify-between rounded-xl border px-3 py-2 text-left transition active:scale-[0.99]',
					chapter.isRead
			...
			>
				<div class="min-w-0">
					<p class="truncate text-sm font-medium text-zinc-200">Глава {{ chapter.number }} — {{ chapter.title }}</p>
					<p class="text-xs text-zinc-500">{{ chapter.publishedAt }}</p>
				</div>
				<span class="ml-2 shrink-0 text-xs" :class="chapter.isRead ? 'text-emerald-300' : 'text-zinc-400'">
