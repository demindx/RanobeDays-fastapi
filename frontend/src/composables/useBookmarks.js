...
export const useBookmarks = () => {
	...
	const activeBookmark = computed(() => sortedBookmarks.value.find((bookmark) => bookmark.id === activeBookmarkId.value) || null)
	...
	const hasNameConflict = (name, ignoreId = null) => {
		...
		return bookmarks.value.some((bookmark) => bookmark.id !== ignoreId && normalizeName(bookmark.name).toLowerCase() === normalized)
	}
	...
	const renameBookmark = (bookmarkId, nextName) => {
		...
		if (hasNameConflict(normalized, bookmarkId)) return { ok: false, error: 'Закладка с таким названием уже существует.' }
	...
	const createBookmark = (name) => {
		...
		if (hasNameConflict(normalized)) return { ok: false, error: 'Закладка с таким названием уже существует.' }
